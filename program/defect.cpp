...
void prosesQualityControl(vector<Wafer>& w) {
    ...
    // Setiap wafer punya defect rate berbeda
    uniform_real_distribution<> defectRateDist(0.05, 0.80);

    for (auto& wafer : w) {

        double defectRate = defectRateDist(gen);
        bernoulli_distribution d(defectRate);
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                wafer.coreGrid[i][j] = isDefect ? 0 : 1;
        ...
        if (wafer.totalOK >= 75)
            wafer.grade = "i9";
        else if (wafer.totalOK >= 50)
            wafer.grade = "i7";
        else if (wafer.totalOK >= 25)
            wafer.grade = "i5";
        else
            wafer.grade = "i3";
